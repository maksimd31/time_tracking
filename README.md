...
- Гостевой режим без регистрации (привязка по IP, отдельные счётчики и возможность продолжить работу после входа)
...
- Просмотр ежедневной и суммарной статистики, расширенная аналитика для авторизованных пользователей
...
- Анонимные пользователи получают собственные счётчики: система создаёт гостевой профиль, привязанный к IP, и блочно хранит интервалы до 14 дней (значение настраивается переменной `GUEST_ACCOUNT_RETENTION_DAYS`).
- После регистрации или входа данные гостя автоматически переносятся в личный аккаунт, поэтому можно начать без учётной записи и продолжить на любом устройстве.
- Для гостей доступен обзорный режим: дашборд и таймеры работают, а детальная аналитика и синхронизация становятся доступны после авторизации.
...
2. Запустите Celery-воркер:
   ...
   ```
3. (Опционально) Запустите планировщик задач, если хотите использовать автоматическую очистку гостевых аккаунтов:
   ```bash
   celery -A Time_tracking beat -l info
   ```
4. В другом терминале запустите Django-сервер:
   ```bash
...
3. Настройте системные сервисы (пример systemd unit):
   ...
   `/etc/systemd/system/time-tracking-beat.service`
   ```ini
   [Unit]
   Description=Celery Beat for time tracking
   After=network.target redis.service
   ...
   WorkingDirectory=/opt/time_tracking
   EnvironmentFile=/opt/time_tracking/.env
   ExecStart=/opt/time_tracking/.venv/bin/celery -A Time_tracking beat -l info
   Restart=always
   ...
   Включите сервисы `sudo systemctl enable --now time-tracking.service time-tracking-celery.service time-tracking-beat.service`.
...
## 📬 Обратная связь
- В интерфейсе присутствует страница [/contacts/](http://127.0.0.1:8000/contacts/) с актуальными способами связи.
- Базовые контакты: `maksimdis31@icloud.com`, Telegram [@The_Chef12](https://t.me/The_Chef12), GitHub [maksimd31](https://github.com/maksimd31).
- Буду рад отзывам о гостевом режиме и идеям для новых метрик — это помогает развивать сервис быстрее.
